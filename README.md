...
| 版本 | 说明 |
|---|---|
| 1.3.0（versionCode 5） | 每日一题；判对无解与失败结算补全；历史最佳显示难度和分数；每日结算不再被底栏挤压 |
| 1.2.0（versionCode 4） | 分享带上题目和算式；中等及以上死步可撤销；计时提示按难度扣分；统计按难度分列 |
| 1.1.0（versionCode 3） | 超难难度（1–13 窄解）、成功结算展示玩家算式、五位数分数时倒计时不再换行 |
